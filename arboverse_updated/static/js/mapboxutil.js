...
document.addEventListener('DOMContentLoaded', () => {
    ...
    map.on('load', function () {
        ...
        // Version from arboverse main page
          /*function showSuggestions(inputElement, suggestions, autocompleteBox) {
            // Clear previous suggestions
            autocompleteBox.innerHTML = '';

            if (suggestions.length === 0) {
              autocompleteBox.classList.add('hidden'); // Hide if no suggestions
              return;
            }

            autocompleteBox.classList.remove('hidden'); // Show if there are suggestions
            ...
            suggestions.forEach(suggestion => {
              ...
              // When clicked, update the input value
              div.onclick = function() {
                inputElement.value = suggestion;
                autocompleteBox.innerHTML = ''; // Clear suggestions after selection
                autocompleteBox.classList.add('hidden'); // Hide suggestions
              };
